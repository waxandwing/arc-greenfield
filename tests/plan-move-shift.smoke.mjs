...
async function pageFlow(browser, data, context) {
  ...
  assert(await page.getByTestId('plan-move-panel-layer').isVisible(), 'Lesson Focus did not open governed Move panel overlay.')
  assert(await page.getByTestId('plan-move-panel').isVisible(), 'Lesson Focus did not open governed Move panel.')
