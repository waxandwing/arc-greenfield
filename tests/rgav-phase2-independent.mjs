...
try {
  ...
  await context.close()
  console.log('Independent Phase 2 RGAV B passed: B01 Settings-owned view/preferences → alternate teacher story/viewport verified cross-view truth, keyboard move + reload, Section divergence, recovery/fixed-anchor context, Section isolation, Apply/Undo persistence, no drag-only route, overflow, and runtime cleanliness.')
} finally {
  await browser.close()
}
