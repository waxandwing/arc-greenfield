...
try {
  ...
  await withPage(browser, storageEntries(data, dayContext), async (page) => {
    ...
    assert(await page.getByRole('button', { name: 'IDEAS', exact: true }).getAttribute('aria-expanded') === 'true', 'Year did not open Tray as an overlay.')
    assert(await page.locator('.plan-state-header').getAttribute('data-plan-overlay') === 'workspace', 'Tray overlay was not reflected from Year.')
    assert(await page.locator('.plan-state-header').getAttribute('data-plan-view') === 'Year Map', 'Opening Tray from Year mutated Plan view.')
    assert(await page.locator('.plan-state-header').getAttribute('data-plan-date') === '2026-09-15', 'Opening Tray from Year mutated the date.')
    assert(await page.getByLabel('IDEAS furniture').getByText('Museum label mini-lesson', { exact: true }).isVisible(), 'Tray did not open over Year.')
    await shot(page, '05-year-workspace.png')
    await page.getByRole('button', { name: 'Close IDEAS', exact: true }).click()
    assert(await page.locator('.plan-state-header').getAttribute('data-plan-overlay') === 'none', 'Closing Tray from Year left overlay state.')
    assert(await page.locator('.plan-state-header').getAttribute('data-plan-view') === 'Year Map', 'Closing Tray from Year did not restore Year.')
    assert(await page.locator('.plan-state-header').getAttribute('data-plan-date') === '2026-09-15', 'Closing Tray from Year moved the date.')
    assert(await page.locator('.plan-state-header').getAttribute('data-plan-course') === 'course-2d', 'Closing Tray from Year lost Course.')
  ...
  console.log('Arc Plan Year context gate passed: Day/Class/Lesson/Week/Month entry, Unit→Month, Tray overlay, Home, refresh, stale IDs, ArcTable isolation.')
} finally {
  await browser.close()
}
