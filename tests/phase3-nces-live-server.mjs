  ...
  try {
    ...
    if (url.pathname === '/api/nces') {
      ...
      const result = await fetchNcesProxy({
        schoolName: url.searchParams.get('schoolName') || '',
        city: url.searchParams.get('city') || '',
        state: url.searchParams.get('state') || '',
        districtName: url.searchParams.get('districtName') || '',
...
server.listen(port, '127.0.0.1', () => {
  console.log(`phase3 NCES live server listening on http://127.0.0.1:${port}`)
})
