...
try {
  ...
  assert(await page.getByRole('heading', { name: 'Planning period' }).isVisible(), 'Closing Workspace did not restore Planning period.')
  assert(await page.locator('.day-continuity').getAttribute('data-plan-date') === '2026-09-15', 'Workspace close moved the anchor date.')
  await shot(page, '07-planning-after-workspace.png')

  assert(runtimeErrors.length === 0, `Plan P5 runtime errors: ${runtimeErrors.join(' | ')}`)
  await context.close()
  console.log('Plan P5 planning-period gate passed: Teaching Day anchor → Planning period buckets → deep links → return → Workspace overlay restore.')
} finally {
  await browser.close()
}
