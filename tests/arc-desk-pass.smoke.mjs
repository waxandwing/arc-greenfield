...
try {
  ...
  assert(await page.getByTestId('desk-source-ideas-drawer').count() === 1, 'IDEAS drawer must use green-folders-drawer.svg source art (not comp crop) by default.')
  const ideasDrawerBg = await page.locator('.arc-desk-green-folders-drawer[data-desk-slices="false"] .arc-desk-green-drawer-art').evaluate(
    (el) => getComputedStyle(el).backgroundImage,
  )
  assert(ideasDrawerBg.includes('green-folders-drawer'), 'IDEAS shell must reference committed drawer SVG.')
  assert(await page.getByTestId('desk-source-todos-body').count() === 1, 'TO-DOS folder must use CSS source chrome by default.')
  assert(await page.getByTestId('desk-slice-ideas-drawer').count() === 0, 'Comp-crop slices must stay off unless deskSlices=1 or VITE_ARC_DESK_SLICES=true.')
  ...
  assert((await edgeTabs.getAttribute('data-desk-slices')) === 'false', 'Planner edge tabs must use CSS source chrome by default (not comp crops).')
  const weekTabArt = await edgeTabs.locator('[data-desk-slice-tab="week"]').evaluate((el) => getComputedStyle(el).backgroundImage)
  assert(!weekTabArt.includes('planner-edge-tab'), 'WEEK tab must not use slice rasters when source mode is default.')
  const titleMarkSource = await page.getByTestId('desk-planner-rainbow-mark').getAttribute('data-desk-mark-source')
  assert(titleMarkSource === 'vector', 'Week title mark must use ArcTable vector source, not comp crop PNG.')
