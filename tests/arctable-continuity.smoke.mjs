...
try {
  ...
  assert(await page.getByRole('button').count() === 0, 'Student Surface must not contain visible or hidden teacher controls.')
  ...
  await page.keyboard.press('Escape')
  assert(await page.getByText('Teacher controls', { exact: true }).count() === 1, 'Escape must return the teacher to Teacher Monitor without adding a control to the projected canvas.')
  ...
  assert(await page.locator('.arctable--student button').count() === 0, 'Projected surface must contain no hidden keyboard-reachable teacher controls.')
  await page.setViewportSize({ width: 390, height: 844 })
