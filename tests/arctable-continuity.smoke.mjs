...
try {
  ...
  assert(await page.getByRole('button', { name: 'Teacher mode' }).count() === 1, 'Student Surface must expose Teacher mode to return to Teacher Monitor.')
  assert(await page.getByRole('button', { name: 'Sync' }).count() === 1, 'Student Surface must expose Sync to follow teacher live state.')
  ...
  assert(await page.getByRole('button', { name: 'Teacher mode' }).count() === 1, 'Projected surface keeps Teacher mode chrome in the header.')
  assert(await page.getByRole('button', { name: 'Sync' }).count() === 1, 'Projected surface keeps Sync chrome in the header.')
