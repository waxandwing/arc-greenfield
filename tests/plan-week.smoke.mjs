...
try {
  ...
  await withPage(browser, storageEntries(data, dayContext), async (page, runtimeErrors) => {
    ...
    assert(await page.getByText('Teaching week', { exact: true }).isVisible(), 'Week state header did not name Teaching week.')
  ...
  await withPage(browser, storageEntries(data, dayContext), async (page) => {
    ...
    assert(await page.getByText('Teaching week', { exact: true }).isVisible(), 'Class → Week header must keep Teaching week as calendar title.')
    assert(await page.getByText(/2D Art 1/).isVisible(), 'Class → Week must still show course context in the grid or kicker.')
  ...
  await withPage(browser, storageEntries(data, dayContext), async (page) => {
    ...
    assert(await page.getByRole('button', { name: 'IDEAS', exact: true }).getAttribute('aria-expanded') === 'true', 'Week did not open Tray as an overlay.')
    ...
    assert(await page.getByLabel('IDEAS furniture').getByText('Museum label mini-lesson', { exact: true }).isVisible(), 'Tray did not open over Week.')
