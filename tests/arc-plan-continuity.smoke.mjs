...
try {
  ...
  await selectView(page, 'Day')
  assert(await page.getByRole('button', { name: 'IDEAS', exact: true }).isVisible() && await page.getByRole('button', { name: 'SETTINGS', exact: true }).isVisible(), 'Arc Plan core controls require ArcTable unexpectedly.')
