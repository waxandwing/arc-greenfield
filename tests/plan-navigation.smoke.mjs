...
try {
  ...
  await withPage(browser, storageEntries(data, dayContext), async (page, runtimeErrors) => {
    ...
    assert(await page.getByRole('button', { name: 'IDEAS', exact: true }).getAttribute('aria-expanded') === 'true', 'Class Focus did not open Tray as an overlay.')
    ...
    await page.getByRole('button', { name: 'IDEAS', exact: true }).click()
    assert(await page.getByLabel('IDEAS furniture').getByText('Museum label mini-lesson', { exact: true }).isVisible(), 'Tray did not open over Lesson Focus.')
  ...
  await withPage(browser, storageEntries(data, dayContext), async (page) => {
    ...
    await page.getByRole('button', { name: 'Open lesson', exact: true }).first().click()
    await page.getByRole('button', { name: 'IDEAS', exact: true }).click()
    assert(await page.getByRole('button', { name: 'IDEAS', exact: true }).getAttribute('aria-expanded') === 'true', 'Tray did not open before Home.')
    await retreatToTeachingDayViaDayTab(page)
    assert(await page.getByRole('button', { name: 'IDEAS', exact: true }).getAttribute('aria-expanded') === 'false', 'Home from Tray did not close the overlay.')
