...
try {
  ...
  await withPage(browser, storageEntries(data, dayContext), async (page, runtimeErrors) => {
    ...
    assert(await page.locator('.day-continuity').getAttribute('data-plan-focus') === 'class', 'Closing Workspace from Class did not restore Class Focus.')
    assert(await page.locator('.day-continuity').getAttribute('data-plan-section') === 'section-p6', 'Closing Workspace from Class lost Section.')
    assert(await page.evaluate(() => localStorage.getItem('arc.arctable.live.v1')) === liveBeforeClassWorkspace, 'Workspace overlay mutated ArcTable live storage.')
    ...
    await page.getByRole('button', { name: 'Close Workspace', exact: true }).click()
    assert(await page.locator('.day-continuity').getAttribute('data-plan-focus') === 'lesson', 'Closing Workspace did not restore Lesson Focus.')
    assert(await page.locator('[data-lesson-focus="lesson-2d-5"]').count() === 1, 'Closing Workspace reconstructed a different Lesson.')
    assert(await page.locator('.plan-state-header').getAttribute('data-plan-overlay') === 'none', 'Workspace overlay remained after Close.')
  ...
  await withPage(browser, storageEntries(data, dayContext), async (page) => {
    ...
    await page.getByRole('button', { name: 'Open Workspace', exact: true }).click()
    assert(await page.getByRole('button', { name: 'Workspace', exact: true }).getAttribute('aria-expanded') === 'true', 'Week did not open Workspace as an overlay.')
    assert(await page.locator('.plan-state-header').getAttribute('data-plan-overlay') === 'workspace', 'Workspace overlay was not reflected from Week.')
    assert(await page.locator('.plan-state-header').getAttribute('data-plan-view') === 'Week', 'Opening Workspace from Week mutated Plan view.')
    assert(await page.locator('.plan-state-header').getAttribute('data-plan-date') === '2026-09-15', 'Opening Workspace from Week mutated the date.')
    assert(await page.getByText('Museum label mini-lesson', { exact: true }).isVisible(), 'Workspace did not open over Week.')
    await shot(page, '04-week-workspace.png')
    await page.getByRole('button', { name: 'Close Workspace', exact: true }).click()
    assert(await page.locator('.plan-state-header').getAttribute('data-plan-overlay') === 'none', 'Closing Workspace from Week left overlay state.')
    assert(await page.locator('.plan-state-header').getAttribute('data-plan-view') === 'Week', 'Closing Workspace from Week did not restore Week.')
    assert(await page.locator('.plan-state-header').getAttribute('data-plan-date') === '2026-09-15', 'Closing Workspace from Week moved the date.')
    assert(await page.locator('.plan-state-header').getAttribute('data-plan-course') === 'course-2d', 'Closing Workspace from Week lost Course.')
    assert(await page.locator('.plan-state-header').getAttribute('data-plan-section') === 'section-p6', 'Closing Workspace from Week lost Section.')
  ...
  await withPage(browser, storageEntries(data, dayContext), async (page) => {
    ...
    assert(await page.getByRole('heading', { level: 1, name: 'Day' }).isVisible(), 'Week → Home did not open the Day view.')
  ...
  console.log('Arc Plan navigation spine gate passed: Day → Class → Lesson → Workspace return, Home to Teaching Day, Week context, refresh, stale IDs, Section divergence, ArcTable isolation.')
} finally {
  await browser.close()
}
