...
try {
  ...
  await withPage(browser, storageEntries(data, dayContext), async (page) => {
    ...
    assert(await page.getByRole('heading', { level: 1, name: 'Day' }).isVisible(), 'Month → Day did not open the Day rail.')
  ...
  await withPage(browser, storageEntries(data, dayContext), async (page) => {
    ...
    assert(await page.getByText('Museum label mini-lesson', { exact: true }).isVisible(), 'Workspace did not open over Month.')
    await shot(page, '04-month-workspace.png')
    await page.getByRole('button', { name: 'Close Workspace', exact: true }).click()
    assert(await page.locator('.plan-state-header').getAttribute('data-plan-overlay') === 'none', 'Closing Workspace from Month left overlay state.')
    assert(await page.locator('.plan-state-header').getAttribute('data-plan-view') === 'Month', 'Closing Workspace from Month did not restore Month.')
    assert(await page.locator('.plan-state-header').getAttribute('data-plan-date') === '2026-09-15', 'Closing Workspace from Month moved the date.')
    assert(await page.locator('.plan-state-header').getAttribute('data-plan-course') === 'course-2d', 'Closing Workspace from Month lost Course.')
  ...
  await withPage(browser, storageEntries(data, dayContext), async (page) => {
    ...
    assert(await page.getByRole('heading', { level: 1, name: 'Day' }).isVisible(), 'Month → Home did not open the Day view.')
  ...
  console.log('Arc Plan Month context gate passed: Day/Class/Lesson entry, selected-date return, Workspace overlay, Home, refresh, stale IDs, Section divergence, ArcTable isolation.')
} finally {
  await browser.close()
}
