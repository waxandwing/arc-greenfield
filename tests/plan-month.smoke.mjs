...
try {
  ...
  await withPage(browser, storageEntries(data, dayContext), async (page) => {
    ...
    assert(await page.getByRole('button', { name: 'IDEAS', exact: true }).getAttribute('aria-expanded') === 'true', 'Month did not open Tray as an overlay.')
    ...
    assert(await page.getByLabel('IDEAS furniture').getByText('Museum label mini-lesson', { exact: true }).isVisible(), 'Tray did not open over Month.')
