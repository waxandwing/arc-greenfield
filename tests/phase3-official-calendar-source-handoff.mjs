...
const choose = page.getByRole('button', { name: 'Load this school' })
await choose.focus()
...
await page.getByLabel('Official calendar link', { exact: true }).fill('javascript:alert(1)')
await page.getByLabel('Published by', { exact: true }).fill('Orange County Public Schools')
await page.getByLabel('Source label', { exact: true }).fill('2026–27 school calendar')
...
assert(heldText?.includes('Your calendar stays untouched until you review and use it.'), 'Held source did not preserve the no-commit-before-review boundary.')
...
await page.getByLabel('Official calendar link', { exact: true }).fill('https://district.example.invalid/revised-calendar.pdf')
