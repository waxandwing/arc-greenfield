...
async function openSchoolCalendarFromSettings(page) {
  const drawer = page.getByRole('complementary', { name: 'Settings and setup' })
  if (!(await drawer.isVisible().catch(() => false))) {
    await page.getByRole('button', { name: 'Settings', exact: true }).click()
  }
  await drawer.waitFor({ state: 'visible' })
  const action = drawer.getByRole('button', { name: 'School calendar', exact: true })
  assert(await action.count() === 1, 'Source review: Settings did not expose exactly one School calendar action.')
  await action.click()
}
...
await openSchoolCalendarFromSettings(page)
