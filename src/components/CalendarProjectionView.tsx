...
import { PlanningWeekDayView, type PlanningWeekObjectActions } from './PlanningWeekDayView'
...
export function CalendarProjectionView({ view, calendar, anchorDate, planningContext, showWeekends = false, weekObjectActions }: Props) {
  ...
  switch (view) {
    case 'Day': {
      ...
      return <PlanningDayStrip title={formatLongDate(projection.date)} day={projection.day} planningContext={planningContext} termContext={<TermContext quarters={projection.quarter ? [projection.quarter] : []} semesters={projection.semester ? [projection.semester] : []} />} />
    }
    case 'Week': {
      ...
      return <PlanningWeekStrip title={formatDateRange(visibleDays[0]?.date ?? projection.startDate, visibleDays[visibleDays.length - 1]?.date ?? projection.endDate)} days={visibleDays} planningContext={planningContext} termContext={<TermContext quarters={projection.quarters} semesters={projection.semesters} />} actions={weekObjectActions} />
    }
    case 'Month': {
      const projection = projectMonth(calendar, anchorDate)
      const monthPlanning = planningContext ? projectMonthPlanning({ month: projection, planning: planningContext.planning, units: planningContext.units, lessons: planningContext.lessons, overrides: planningContext.shiftState?.overrides ?? [] }) : null
      return (
        <section className="projection-section month-section" aria-label={`${formatMonth(anchorDate)} calendar`}>
          <div className="projection-heading-row"><p className="projection-range-label">{formatMonth(anchorDate)}</p><TermContext quarters={projection.quarters} semesters={projection.semesters} /></div>
          {monthPlanning ? <div className="planning-scroll-frame"><PlanningMonthView month={projection} planning={monthPlanning} /></div> : <CalendarOnlyMonth projection={projection} label={formatMonth(anchorDate)} />}
        </section>
      )
    }
    case 'Quarter': {
...
function CalendarOnlyMonth({ projection, label }: { projection: ReturnType<typeof projectMonth>; label: string }) {
  return (
    <>
      <div className="month-weekday-row" aria-hidden="true">{WEEKDAY_LABELS.map((weekday) => <span key={weekday}>{weekday}</span>)}</div>
      <div className="month-projection" role="region" aria-label={`${label} calendar grid`}>{projection.weeks.map((week) => <div className="month-week" key={week.startDate}>{week.days.map((day) => <CalendarDayCell key={day.date} day={day} />)}</div>)}</div>
    </>
  )
}

function planningForDays(days: ProjectedDay[], context: PlanningContext) {
  return projectPlanningRange({ dates: days.map((day) => day.date), planning: context.planning, units: context.units, lessons: context.lessons, overrides: context.shiftState?.overrides ?? [] })
}
