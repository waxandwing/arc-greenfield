...
export function PlanningWeekDayView({
  ...
  return (
    <div
      ...
      onKeyDown={(event) => {
        if (event.key === 'Escape' && selected && !quickAdd) {
          setSelected(null)
        }
      }}
    >
...
function PlanningDateHeader({
  ...
  days: ProjectedDay[]
  single: boolean
  onQuickAdd?: (date: ISODate, trigger: HTMLButtonElement) => void
}) {
  return (
      ...
      {days.map((day) => (
        <div key={day.date} className={`planning-date-heading planning-date-heading--${day.kind}`}>
          ...
          {onQuickAdd && day.kind === 'instructional' ? (
            <button
              type="button"
              className="planning-quick-add-trigger"
              aria-label={`Add work on ${formatLongDate(day.date)}`}
              onClick={(event) => onQuickAdd(day.date, event.currentTarget)}
            >+</button>
          ) : null}
        </div>
      ))}
...
function QuickAddComposer({ date, actions, onDismiss }: { date: ISODate; actions?: PlanningWeekObjectActions; onDismiss: () => void }) {
  ...
  useEffect(() => {
    if (!lessonCourses.some((course) => course.id === lessonCourseId)) setLessonCourseId(lessonCourses[0]?.id ?? '')
  }, [lessonCourseId, lessonCourses])
  useEffect(() => {
    if (kind === 'lesson' && !lessonUnits.some((unit) => unit.id === unitId)) setUnitId(lessonUnits[0]?.id ?? '')
  }, [kind, lessonUnits, unitId])
  useEffect(() => { setEndDate(date) }, [date])

  const invalidUnitRange = kind === 'unit' && endDate < date
  const disabled = !title.trim()
    || (kind === 'unit' && (!courseId || invalidUnitRange))
    || (kind === 'lesson' && !unitId)
  ...
  return (
    <section
      ...
      role="dialog"
      aria-modal="false"
      aria-label={`Add work on ${formatLongDate(date)}`}
      onKeyDown={(event) => {
        if (event.key === 'Escape') {
    ...
    >
      <div className="planning-quick-add-heading">
        <strong>Add to {formatShortDate(date)}</strong>
        <button type="button" className="planning-context-dismiss" aria-label="Close quick add" onClick={onDismiss}>×</button>
      </div>
      <div className="planning-quick-add-kinds" role="group" aria-label="Object type">
        {(['unit', 'lesson', 'note'] as const).map((candidate) => (
          <button key={candidate} type="button" aria-pressed={kind === candidate} onClick={() => setKind(candidate)}>
            {candidate === 'unit' ? 'Unit' : candidate === 'lesson' ? 'Lesson' : 'Note'}
          </button>
        ))}
      </div>
      {kind === 'note' ? (
        <textarea ref={titleRef as React.RefObject<HTMLTextAreaElement>} aria-label="Note text" rows={3} value={title} onChange={(event) => setTitle(event.target.value)} />
      ) : (
        <input ref={titleRef as React.RefObject<HTMLInputElement>} aria-label={kind === 'unit' ? 'Unit title' : 'Lesson title'} value={title} onChange={(event) => setTitle(event.target.value)} />
      )}
      {kind === 'unit' ? (
        <>
          <label>Course<select aria-label="Course" value={courseId} onChange={(event) => setCourseId(event.target.value)}>{courses.map((course) => <option key={course.id} value={course.id}>{course.title}</option>)}</select></label>
          <label>Through<input type="date" aria-label="Unit end date" min={date} value={endDate} onChange={(event) => setEndDate(event.target.value as ISODate)} /></label>
          {invalidUnitRange ? <span className="planning-action-hint" role="status">Unit end date cannot be before its start date.</span> : null}
        </>
      ) : null}
      {kind === 'lesson' ? (
        <>
          {lessonCourses.length > 1 ? <label>Course<select aria-label="Lesson Course" value={lessonCourseId} onChange={(event) => setLessonCourseId(event.target.value)}>{lessonCourses.map((course) => <option key={course.id} value={course.id}>{course.title}</option>)}</select></label> : null}
          <label>Unit<select aria-label="Unit" value={unitId} onChange={(event) => setUnitId(event.target.value)}>{lessonUnits.map((unit) => <option key={unit.id} value={unit.id}>{unit.title}</option>)}</select>{eligibleUnits.length === 0 ? <span className="planning-action-hint">Place a Unit across this date before adding a Lesson.</span> : null}</label>
        </>
      ) : null}
      {kind === 'note' ? (
        <div className="planning-quick-add-note-options">
          <label>Placement<select aria-label="Note placement" value={placement} onChange={(event) => setPlacement(event.target.value as PlanningNotePlacement)}><option value="calendar">Notes</option><option value="after-school">After School</option></select></label>
          <label className="planning-check"><input type="checkbox" aria-label="Important" checked={important} onChange={(event) => setImportant(event.target.checked)} />Important</label>
        </div>
      ) : null}
...
function NoteTile({ note, selected, onSelect, actions, onDismiss }: { note: PlanningNoteProjection; selected: boolean; onSelect: () => void; actions?: PlanningWeekObjectActions; onDismiss: () => void }) {
  ...
  return (
    <article className={`planning-note${note.important ? ' planning-note--important' : ''}${selected ? ' planning-object--selected' : ''}`} role="listitem">
      <button ref={buttonRef} type="button" className="planning-object-select planning-note-select" aria-label={`Select ${accessible}`} aria-pressed={selected} onClick={onSelect}>
      ...
      {selected ? (
        <>
          <ContextToolbar label="Note actions" onDismiss={dismissAndFocus} actions={[
          ...
          <div className="planning-section-row" style={gridTemplate(days.length)} key={row.section.id}>
            ...
            {row.days.map((slot, index) => (
              <div key={slot.date} className={`planning-day-slot planning-day-slot--${days[index]?.kind ?? 'unknown'}`} aria-label={`${row.section.name}, ${formatLongDate(slot.date)}`} role={slot.lessons.length > 0 ? 'list' : undefined}>
                {slot.lessons.map((lesson) => <LessonTile key={lesson.lessonId} lesson={lesson} courseId={course.course.id} sectionId={row.section.id} selected={selected?.kind === 'lesson' && selected.id === lesson.lessonId && selected.sectionId === row.section.id} onSelect={() => onSelect({ kind: 'lesson', id: lesson.lessonId, sectionId: row.section.id })} actions={actions} onDismiss={onDismiss} />)}
...
function UnitTile({ courseId, unit, index, days, selected, onSelect, actions, onDismiss }: { courseId: string; unit: PlanningCourseGroup['unitSpans'][number]; index: number; days: ProjectedDay[]; selected: boolean; onSelect: () => void; actions?: PlanningWeekObjectActions; onDismiss: () => void }) {
  ...
  return (
    <div className="planning-unit-grid" style={gridTemplate(days.length)}>
      <span className="planning-row-label planning-row-label--unit">{index === 0 ? 'Unit' : ''}</span>
      <div className={`planning-unit-object${selected ? ' planning-object--selected' : ''}`} style={{ gridColumn: `${unit.startIndex + 2} / ${unit.endIndex + 3}` }}>
        <button ref={buttonRef} id={unitSpanId(courseId, unit.unitId)} type="button" className="planning-unit-span planning-object-select" title={`${unit.title}: ${unit.startDate} through ${unit.endDate}`} aria-label={`Select Unit ${unit.title}, ${formatLongDate(unit.startDate)} through ${formatLongDate(unit.endDate)}`} aria-pressed={selected} onClick={onSelect}>{unit.title}</button>
        {selected ? (
          <>
            <ContextToolbar label={`${unit.title} actions`} onDismiss={dismissAndFocus} actions={[
            ...
            {moving ? <InlineEditor label="Move or resize Unit" onCancel={() => setMoving(false)}><label>Start<input type="date" aria-label="Unit start date" value={startDate} onChange={(event) => setStartDate(event.target.value as ISODate)} /></label><label>End<input type="date" aria-label="Unit end date" value={endDate} onChange={(event) => setEndDate(event.target.value as ISODate)} /></label>{invalidRange ? <span className="planning-action-hint" role="status">Unit end date cannot be before its start date.</span> : null}<button type="button" className="planning-action-primary" disabled={invalidRange} onClick={() => { actions?.moveUnit?.(unit.unitId, startDate, endDate); setMoving(false) }}>Apply range</button></InlineEditor> : null}
          </>
        ) : null}
      </div>
    </div>
...
function LessonTile({ lesson, courseId, sectionId, selected, onSelect, actions, onDismiss }: { lesson: PlanningLessonPlacement; courseId: string; sectionId: string; selected: boolean; onSelect: () => void; actions?: PlanningWeekObjectActions; onDismiss: () => void }) {
  ...
  return (
    <article className={`planning-lesson planning-lesson--${lesson.deliveryStatus}${lesson.datePolicy === 'fixed' ? ' planning-lesson--fixed' : ''}${selected ? ' planning-object--selected' : ''}`} aria-describedby={unitSpanId(courseId, lesson.unitId)} role="listitem">
      <button ref={buttonRef} type="button" className="planning-object-select planning-lesson-select" aria-label={`Select ${accessible}`} aria-pressed={selected} onClick={onSelect}>
      ...
      {selected ? (
        <>
          <ContextToolbar label={`${lesson.title} actions`} onDismiss={dismissAndFocus} actions={[
          ...
          {moving ? <InlineEditor label="Move Lesson shared plan" onCancel={() => setMoving(false)}><p className="planning-action-hint">Moves the shared Course plan. Section-specific Shift dates remain separate.</p><label>Date<input type="date" aria-label="Lesson planned date" value={targetDate} onChange={(event) => setTargetDate(event.target.value as ISODate)} /></label><button type="button" className="planning-action-primary" onClick={() => { actions?.moveLesson?.(lesson.lessonId, targetDate); setMoving(false) }}>Move Lesson</button></InlineEditor> : null}
          {shifting ? <InlineEditor label="Shift Lesson for this class" onCancel={() => { setShifting(false); setShiftPreview(null) }}><p className="planning-action-hint">Changes only this Section. The shared Course plan is not moved.</p><label>Date<input type="date" aria-label="Shift destination date" value={targetDate} onChange={(event) => { setTargetDate(event.target.value as ISODate); setShiftPreview(null) }} /></label>{shiftPreview ? <p className={shiftPreview.allowed ? 'planning-shift-preview' : 'planning-shift-preview planning-shift-preview--blocked'} role="status">{shiftPreview.message}</p> : null}{!shiftPreview ? <button type="button" className="planning-action-primary" onClick={() => setShiftPreview(actions?.previewShift?.(sectionId, lesson.lessonId, lesson.effectiveDate, targetDate) ?? { allowed: false, message: 'Shift preview is unavailable.' })}>Preview Shift</button> : null}{shiftPreview?.allowed ? <button type="button" className="planning-action-primary" onClick={() => { actions?.applyShift?.(sectionId, lesson.lessonId, lesson.effectiveDate, targetDate); setShifting(false); setShiftPreview(null) }}>Apply Shift</button> : null}</InlineEditor> : null}
        </>
      ) : null}
    </article>
  )
...
function ContextToolbar({ label, actions, onDismiss }: { label: string; actions: ToolbarAction[]; onDismiss: () => void }) {
  ...
  return (
    <div ref={toolbarRef} className="planning-context-toolbar" role="toolbar" aria-label={label} onKeyDown={handleKeyDown}>
      {available.map((action) => {
        const needsConfirmation = action.destructive && confirming !== action.label
        const visibleLabel = action.destructive && confirming === action.label ? `Confirm ${action.label}` : action.label
        return (
          <button
            key={action.label}
            type="button"
            className={action.destructive ? 'planning-context-action planning-context-action--danger' : 'planning-context-action'}
            onClick={(event) => {
              event.stopPropagation()
...
function InlineEditor({ label, children, onCancel }: { label: string; children: React.ReactNode; onCancel: () => void }) {
  return (
    <div className="planning-inline-editor" role="group" aria-label={label} onKeyDown={(event) => {
      if (event.key === 'Escape') {
        event.preventDefault()
        event.stopPropagation()
        onCancel()
      ...
      <div className="planning-inline-editor-heading"><strong>{label}</strong><button type="button" className="planning-context-dismiss" aria-label={`Close ${label}`} onClick={onCancel}>×</button></div>
      {children}
    </div>
  )
}
