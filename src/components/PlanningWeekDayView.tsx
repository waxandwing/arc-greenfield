...
function NoteTile({ note, selected, onSelect, actions, onDismiss }: { note: PlanningNoteProjection; selected: boolean; onSelect: () => void; actions?: PlanningWeekObjectActions; onDismiss: () => void }) {
  ...
  useEffect(() => {
    if (pendingMove && note.date === pendingMove.date && note.placement === pendingMove.placement) { buttonRef.current?.focus(); setMoving(false); setPendingMove(null) }
...
function LessonTile({ lesson, courseId, sectionId, selected, onSelect, actions, onDismiss }: { lesson: PlanningLessonPlacement; courseId: string; sectionId: string; selected: boolean; onSelect: () => void; actions?: PlanningWeekObjectActions; onDismiss: () => void }) {
  ...
  useEffect(() => { if (pendingMove && lesson.sharedPlannedDate === pendingMove) { buttonRef.current?.focus(); setMoving(false); setPendingMove(null) } }, [lesson.sharedPlannedDate, pendingMove])
  useEffect(() => { if (pendingShift && lesson.effectiveDate === pendingShift) { buttonRef.current?.focus(); setShifting(false); setShiftPreview(null); setPendingShift(null) } }, [lesson.effectiveDate, pendingShift])
  const dismissAndFocus = () => { onDismiss(); requestAnimationFrame(() => buttonRef.current?.focus()) }
