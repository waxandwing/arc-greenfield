...
export function ArcOnboarding(props: Props) {
  ...
  if (stage === 'welcome') {
    return (
      ...
      </section><section id="start-simple" className="onboarding-simple" aria-label="Start simple"><CalendarSetup initialValue={props.draft.calendarDraft ?? props.calendarInput} onDraftChange={(calendarDraft) => props.onChangeDraft({ ...props.draft, calendarDraft })} onSchoolIdentitySelected={(candidate) => props.onChangeDraft({ ...props.draft, schoolNcesId: candidate.id })} loadedSchoolNcesId={props.draft.schoolNcesId} onSave={(calendar, input) => { if (props.onUseCalendar(calendar, input)) props.onChangeDraft({ ...props.draft, stage: 'landed', dismissed: true, calendarDraft: undefined }) }} /></section></div>
    )
  }
  if (stage === 'calendar') {
    return <div className="onboarding-stage"><OnboardingBack onClick={() => props.onChangeDraft({ ...props.draft, stage: 'welcome' })} /><CalendarSetup initialValue={props.draft.calendarDraft ?? props.calendarInput} onDraftChange={(calendarDraft) => props.onChangeDraft({ ...props.draft, calendarDraft })} onSchoolIdentitySelected={(candidate) => props.onChangeDraft({ ...props.draft, schoolNcesId: candidate.id })} loadedSchoolNcesId={props.draft.schoolNcesId} onSave={(calendar, input) => { if (!props.onUseCalendar(calendar, input)) return; if (props.draft.intent === 'import') { props.onChangeDraft({ ...props.draft, stage: 'landed', dismissed: true, calendarDraft: undefined }); props.onOpenImport() } else props.onChangeDraft({ ...props.draft, stage: 'classes', calendarDraft: undefined }) }} onCancel={props.calendar ? dismissSetup : undefined} /></div>
  }
