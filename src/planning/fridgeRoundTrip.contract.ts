...
const unit = placeUnit(
  createUnit({ id: 'unit-one', calendarId: calendar.id, courseId: course.id, title: 'Unit One' }),
  calendar,
...
const fromFridge = moveLessonFromFridge({
  calendar,
  units,
  lessons: toFridge.lessons,
  overrides: toFridge.overrides,
  lessonId: lesson.id,
...
try {
  moveLessonFromFridge({ calendar, units, lessons: toFridge.lessons, overrides: [], lessonId: lesson.id, plannedDate: '2026-09-19' })
} catch {
