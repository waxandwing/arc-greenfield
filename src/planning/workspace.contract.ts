...
const hydratedNotes = workspace.notes ?? []
...
assert(hydratedNotes.length === 2, 'Workspace must preserve independent Notes.')
assert(hydratedNotes[0].text.includes('\n'), 'Freeform Note line breaks must survive hydration.')
assert(hydratedNotes[1].placement === 'after-school' && hydratedNotes[1].important, 'After School and Important semantics must survive hydration.')
...
const duplicateNote = { ...workspace, notes: [hydratedNotes[0], { ...hydratedNotes[0] }] }
...
const wrongNoteCalendar = { ...workspace, notes: [{ ...hydratedNotes[0], calendarId: 'calendar-other' }] }
