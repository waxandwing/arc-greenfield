...
export function projectPlanningLessonSignals(range: PlanningRangeProjection, date: ISODate): PlanningLessonSignal[] {
    ...
    for (const sectionRow of courseGroup.sections) {
      ...
      for (const lesson of day.lessons) {
        ...
        if (signal.sections.some((scope) => scope.sectionId === sectionRow.section.id)) {
          throw new Error(`Planning signals received duplicate Section placement for ${sectionRow.section.id}, ${lesson.lessonId}, ${date}.`)
        }
  ...
  return [...grouped.values()]
    ...
    .sort((a, b) => a.courseTitle.localeCompare(b.courseTitle) || a.title.localeCompare(b.title) || a.lessonId.localeCompare(b.lessonId))
}
