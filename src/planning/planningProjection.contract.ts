...
const calendar = hydrateSchoolCalendar({ id:'calendar-2026-27', schoolYearLabel:'2026–27', firstDay:'2026-08-10', lastDay:'2027-05-28', instructionalWeekdays:[1,2,3,4,5], patternSource:'manual', patternConfidence:'confirmed', exceptions:[], quarters:[], semesters:[] })
...
const planning:PlanningWorkspace={calendarId:calendar.id,courses:[course],sections:[p2,p5,p7],notes:[
  {id:'note-freeform',calendarId:calendar.id,date:'2026-09-15',text:'Bring image set\nCompare scale + material',placement:'calendar',important:false,sourceLabel:null,sourceLocator:null},
  {id:'note-after-school',calendarId:calendar.id,date:'2026-09-17',text:'Museum club pickup',placement:'after-school',important:true,sourceLabel:'District activities calendar',sourceLocator:'district://activities/2026-09-17'},
  {id:'note-next-week',calendarId:calendar.id,date:'2026-09-21',text:'Outside visible Week',placement:'calendar',important:false,sourceLabel:null,sourceLocator:null},
]}
...
const overrides:SectionLessonDateOverride[]=[{sectionId:p5.id,lessonId:lesson17.id,plannedDate:'2026-09-17'},{sectionId:p5.id,lessonId:lesson18.id,plannedDate:'2026-09-21'}]
...
assert(week.notes.length===2,'Week should project only Notes inside its visible date interval.')
assert(week.notes.some((note)=>note.noteId==='note-freeform'&&note.placement==='calendar'&&note.text.includes('\n')),'Freeform Note should project into the calendar Note lane without losing line breaks.')
assert(week.notes.some((note)=>note.noteId==='note-after-school'&&note.placement==='after-school'&&note.important&&note.sourceLabel==='District activities calendar'),'After School Note should retain Important and provenance semantics.')
...
assert(p2Row.days[3].lessons.length===2,'Multiple distinct same-day Lessons should remain distinct in one Section/day slot.')
...
assert(day.notes.length===1&&day.notes[0].noteId==='note-after-school','Day should expose only the Note on its exact date.')
assert(day.courses[0].sections.find((row)=>row.section.id===p2.id)!.days[0].lessons.length===2,'Day and Week must expose the same same-day Lesson truth.')
...
assert(classesOnly.notes.length===1,'Notes must remain visible independently of Units or Lessons.')
let mismatchRejected=false;try{projectPlanningRange({dates:['2026-09-17'],planning:{...planning,calendarId:'other-calendar'},units,lessons,overrides})}catch{mismatchRejected=true}
