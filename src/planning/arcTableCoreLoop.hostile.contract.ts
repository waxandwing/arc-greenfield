...
const p5Session = projectArcTableSession({ day: freshDay, sectionId: p5.id, lessonId: lesson17.id, calendar, liveDate })

// Same display name is never identity. The exact Section ID remains binding.
const p2Session = projectArcTableSession({ day: freshDay, sectionId: p2.id, lessonId: lesson17.id, calendar, liveDate })
...
rejects(
  ...
  'An ArcTable session opened before a Section Shift must not write against the stale schedule.',
)
...
const afterUnrelatedChange = applyArcTableTeachingOutcome({
  session: p5Session,
  liveDate,
  calendar,
  planning,
...
const afterRename = applyArcTableTeachingOutcome({
  session: p5Session,
  liveDate,
  calendar,
  planning,
...
rejects(
  () => applyArcTableTeachingOutcome({ session: p5Session, liveDate, calendar, planning, units, lessons: fixedLessons, overrides: [], outcome: { kind: 'completed' } }),
...
const carryoverSession = projectArcTableSession({ day: carryoverDay, sectionId: p5.id, lessonId: carryoverLesson.id, calendar, liveDate })
...
rejects(
  () => applyArcTableTeachingOutcome({ session: carryoverSession, liveDate, calendar, planning, units, lessons: carryoverWorkspace, overrides: carryoverNowScheduled, outcome: { kind: 'completed' } }),
  'A carryover that became today’s scheduled work after launch must require reopening ArcTable.',
)
...
rejects(
  () => applyArcTableTeachingOutcome({ session: p5Session, liveDate, calendar, planning, units, lessons: afterStopWorkspace, overrides: [], outcome: { kind: 'completed' } }),
  'Replaying an already-consumed ArcTable session must not overwrite the first teaching outcome.',
)
...
rejects(
  () => applyArcTableTeachingOutcome({ session: p5Session, liveDate, calendar, planning: foreignPlanning, units, lessons: freshLessons, overrides: [], outcome: { kind: 'completed' } }),
  'ArcTable must fail closed when canonical workspace ownership no longer matches the loaded calendar.',
)

// The domain adapter must remain pure with respect to schedule and shared curriculum.
assert(movedOverrides[0].plannedDate === '2026-09-18', 'Rejected ArcTable outcomes must not mutate Section Shift overrides.')
assert(lesson17.plannedDate === liveDate && lesson17.datePolicy === 'flexible', 'ArcTable outcomes must not mutate shared Lesson scheduling truth.')
assert(freshLessons.deliveryStates.length === 0, 'ArcTable outcomes must return a delivery state rather than mutating the source Lesson workspace in place.')

console.log('High-stakes ArcTable core-loop hostile contract passed')
