...
export async function searchNcesPublicSchools(query: SchoolIdentityQuery, options: SearchOptions = {}): Promise<OfficialSourceSearchResult> {
...
function parseNcesFeatureResponse(value: unknown): OfficialSourceSearchResult {
  if (!isRecord(value)) return { status: 'invalid', candidates: [], message: 'NCES returned an unreadable school-directory response.' }
  ...
  if (!Array.isArray(value.features)) return { status: 'invalid', candidates: [], message: 'NCES response did not include a school candidate list.' }
  const candidates: OfficialSourceCandidate[] = []
  for (const feature of value.features) {
    const candidate = parseNcesFeature(feature)
    if (!candidate) return { status: 'invalid', candidates: [], message: 'NCES returned an incomplete school identity record.' }
    candidates.push(candidate)
  }
  return candidates.length > 0 ? { status: 'candidates', candidates } : { status: 'none', candidates: [] }
}
...
function clampCandidateLimit(value: number): number { if (!Number.isFinite(value)) return 25; return Math.max(1, Math.min(50, Math.floor(value))) }
function escapeSql(value: string): string { return value.replaceAll("'", "''") }
function escapeSqlLike(value: string): string { return escapeSql(value).replaceAll('%', '').replaceAll('_', '') }
