...
async function main() {
  ...
  const adapter: OfficialCalendarExtractionAdapter = {
    ...
    extract: async (candidate) => {
      extractedSourceId = candidate.id
      extractedSourceLocator = candidate.locator
      return extraction
    },
  }
  ...
  assert(extractedSourceId === source.id && extractedSourceLocator === source.locator, 'Adapter did not receive the exact teacher-confirmed source candidate.')
