...
import { emptyWorkspace, type Course, type PriorityTier, type Workspace } from "../lib/domain";
...
export function ArcShell({ buildId, gitSha }: { buildId: string; gitSha: string }) {
  ...
  function addPlan(title: string, courseId: string | null, date: string | null, location: "calendar" | "ideas") {
  ...
  function saveCellDraft() {
    if (!cellDraft?.title.trim()) return;
    addPlan(cellDraft.title, cellDraft.courseId, cellDraft.date, "calendar");
    setCellDraft(null);
  }
      ...
      {screen === "setup" ? (
        <section className="setupPage">
          <div className="setupCopy">
            ...
            <p>Arc needs your name, the classes you actually teach, and the rough bounds of your school year. That is enough to begin.</p>
          </div>

          <div className="setupGrid">
            <section className="setupCard">
              <span className="stepNumber">1</span><h2>You</h2>
              <label><span>Your name</span><input value={workspace.teacherName} onChange={(e) => updateWorkspace((current) => ({ ...current, teacherName: e.target.value }))} placeholder="What should Arc call you?" /></label>
            </section>

            <section className="setupCard wideCard">
              <span className="stepNumber">2</span><h2>Classes</h2>
              <div className="courseAdder">
                <input value={draftCourse} onChange={(e) => setDraftCourse(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") addCourse(); }} placeholder="Course name" />
                <input value={draftPeriod} onChange={(e) => setDraftPeriod(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") addCourse(); }} placeholder="Period / block" />
              ...
              <div className="courseChips">
                {workspace.courses.map((course) => (
                  <span className="courseChip" key={course.id} style={{ borderColor: course.color }}>
                    <i style={{ background: course.color }} />{course.name}{course.periodLabel ? ` · ${course.periodLabel}` : ""}
            ...
            <section className="setupCard">
              <span className="stepNumber">3</span><h2>School year</h2>
          ...
          <div className="setupFooter">
            <p>No school dates, quarters, imports, and deeper preferences come later.</p>
                          ...
                          <div className="dayCell" key={day.key}>
                            ...
                            {editing ? (
                              <div className="cellComposer">
                                <input autoFocus value={cellDraft.title} onChange={(e) => setCellDraft({ ...cellDraft, title: e.target.value })} onKeyDown={(e) => { if (e.key === "Enter") saveCellDraft(); if (e.key === "Escape") setCellDraft(null); }} placeholder="Lesson or note" />
                                <button type="button" onClick={saveCellDraft}>Add</button>
                              </div>
                            ) : (
                              <button type="button" className="cellAdd" aria-label={`Add to ${course.name} on ${day.label}`} onClick={() => setCellDraft({ courseId: course.id, date: day.key, title: "" })}>＋</button>
                            )}
                          </div>
                        );
                      })}
            ...
            <aside className="workbench" aria-label="Planning workbench">
              ...
              <section className="priorityPanel" aria-label="Must should could priorities">
                ...
                {(["must", "should", "could"] as const).map((tier) => (
                  <div key={tier} className="priorityTier">
                    <div className="priorityHeading"><span>{tier}</span><button type="button" aria-label={`Add ${tier} priority`} onClick={() => setPriorityDraft({ tier, title: "" })}>＋</button></div>
                    <div className="priorityList">
                      {workspace.priorities.filter((priority) => priority.tier === tier).map((priority) => (
                        <label className={priority.completed ? "priorityItem done" : "priorityItem"} key={priority.id}><input type="checkbox" checked={priority.completed} onChange={() => togglePriority(priority.id)} /><span>{priority.title}</span></label>
                      ))}
                    </div>
                    {priorityDraft?.tier === tier && <div className="priorityComposer"><input autoFocus value={priorityDraft.title} onChange={(e) => setPriorityDraft({ tier, title: e.target.value })} onKeyDown={(e) => { if (e.key === "Enter") addPriority(); if (e.key === "Escape") setPriorityDraft(null); }} placeholder={`Add a ${tier}`} /><button type="button" onClick={addPriority}>Add</button></div>}
                  </div>
                ))}
              </section>
            </aside>
