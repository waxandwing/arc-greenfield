...
export function QuarterView({ workspace, range, courseId, selectedPlanId, pasteTargetDate, onSelectPlan, onSelectDate, onMovePlan, onAddPlan, onNestLesson }: QuarterViewProps) {
  ...
  return <section className="quarterSurface" aria-label={`${range.label} planning view`}>
    ...
    <div className="quarterWeeks">{range.weeks.map((week, index) => <section className="quarterWeek" key={week.key}><header><span>Week {index + 1}</span><small>{week.days[0].date.toLocaleDateString(undefined, { month: "short", day: "numeric" })}</small></header><div className="quarterDays" style={{ gridTemplateColumns: `repeat(${columns},minmax(0,1fr))` }}>{week.days.map((day) => {
      ...
      return <div className={`quarterDay${target ? " pasteTarget" : ""}${weekend ? " weekendDay" : ""}`} key={day.key} onClick={() => onSelectDate(day.key)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const id = event.dataTransfer.getData("text/arc-plan"); if (id) attemptMove(id, day.key); }}>
