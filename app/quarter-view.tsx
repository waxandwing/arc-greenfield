    ...
    <section className="quarterSurface" aria-label={`${range.label} planning view`}>
      <div className="rangeViewHeader">
        <div>
          ...
          <h2>{range.label}</h2>
          <p>{range.start} – {range.end} · Move the Unit as a sequence or select one Lesson on the day it happens.</p>
        </div>
        ...
        {range.weeks.map((week, index) => (
          <section className="quarterWeek" key={week.key}>
            <header>
              ...
              <small>{week.days[0].date.toLocaleDateString(undefined, { month: "short", day: "numeric" })}</small>
            </header>
            <div className="quarterDays">
              {week.days.map((day) => {
                ...
                return (
                  <div
                    className={`quarterDay${target ? " pasteTarget" : ""}`}
                    key={day.key}
                      ...
                      {plans.map((plan) => {
                        ...
                        return (
                          <button
                            type="button"
                            draggable
                            className={`${plan.type === "unit" ? "quarterPlan unit" : "quarterPlan"}${spanPosition ? ` ${spanPosition}` : ""}${selectedPlanId === plan.id ? " selected" : ""}`}
                            key={`${plan.id}-${day.key}`}
                            onDragStart={(event) => {
                      ...
                      {nestedLessons.map((lesson) => {
                        ...
                        return (
                          <button
                            type="button"
                            draggable
                            className={`quarterPlan rangeNestedLesson${selectedPlanId === lesson.id ? " selected" : ""}`}
                            key={lesson.id}
                            onDragStart={(event) => {
                              event.stopPropagation();
