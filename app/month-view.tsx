    ...
    <section className="monthSurface" aria-label={`${label} planning view`}>
      ...
      <div className="monthWeeks">
        {weeks.map((week) => <div className="monthWeek" style={{ gridTemplateColumns: `repeat(${columns},minmax(0,1fr))` }} key={week.key}>{week.days.map((day) => {
          ...
          return <div className={`monthDay${day.inPrimaryMonth ? "" : " outsideMonth"}${target ? " pasteTarget" : ""}${weekend ? " weekendDay" : ""}`} key={day.key} onClick={() => onSelectDate(day.key)} onDragOver={(event) => event.preventDefault()} onDrop={(event) => { event.preventDefault(); const id = event.dataTransfer.getData("text/arc-plan"); if (id) onMovePlan(id, day.key, courseId); }}>
