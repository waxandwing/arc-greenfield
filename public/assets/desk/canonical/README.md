...
| `calendar-class-selected-marker.svg` | `selected class calendar icon vector.svg` | class-focus selection marker | canonical | **BLOCKED** — binary missing (do not confuse with view tabs) |
| `calendar-date-rainbow.svg` | `to left of date on calendar rainbow icon.svg` | accent left of calendar date | canonical | **BLOCKED** — binary missing (live still uses `planner-rainbow-mark.*`) |
| `today-button.svg` | `today button.svg` | Today physical control | canonical | **BLOCKED** — binary missing |
| `today-highlight.svg` | `today highlighter icon.svg` | Today marker/highlighter | canonical | **BLOCKED** — binary missing (CSS semicircle interim only) |
...
| `ideas-tray.svg` | `tray image (use).svg` | IDEAS tray/drawer | **USE / canonical** | **BLOCKED** — binary missing (live still `green-folders-drawer.*`) |
| `ideas-tray-vector-alt.svg` | `Ideas Tray Vector.svg` | simplified/alternate tray | reference only | **BLOCKED** — binary missing |
| `wood-background-light.svg` | `Wood Background Light.svg` | light wood desk surface | canonical candidate | **BLOCKED** — binary missing (live still Icarus `texture-wood.png` + `light-wood-desk.png` for schedule setup) |
